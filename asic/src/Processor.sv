...
// ASIC version: added probe output to prevent the synthesizer from
// optimizing away the design (original has no outputs).
module Processor import signals_pkg::*; #(parameter N = 32, parameter W = 5)
(
    input  logic clk,
    input  logic rst,
    input  logic stall,
    output logic [N-1:0] probe
);
